...
# ============= QUANTIZE THRESHOLD =============
def quantize_threshold(df, bits=64):
    """Lượng tử hóa threshold theo min-max normalization"""
    thresholds = df[df['Threshold'] != 0]['Threshold'].values
    ...
    def normalize(value):
        ...
        quantized = int(normalized * ((2 ** bits) - 1))
        return quantized
    
    df['Threshold_Quant'] = df['Threshold'].apply(normalize)
    return df

df = quantize_threshold(df, bits=64)
